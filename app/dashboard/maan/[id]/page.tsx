...
export default async function MaanLeadPage({ params }: LeadPageProps) {
    ...
    return (
        <div className="container mx-auto py-10 space-y-6">
            <div className="flex items-center gap-4">
                <Link href="/dashboard/maan">
                    <Button variant="outline" size="icon">
                        <ArrowLeft className="h-4 w-4" />
                    </Button>
                </Link>
                <h1 className="text-3xl font-bold tracking-tight">MAAN - Редактирование Лида</h1>
            </div>
            <LeadEditForm lead={lead} history={history} backUrl="/dashboard/maan" />
        </div>
    )
}
