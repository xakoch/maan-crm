...
export async function submitFormLead(formData: {
    name: string
    phone: string
    city?: string
    region?: string
    ...
    try {
        ...
        let managerToSend: any = null
        const isMaan = formData.crmType === 'maan'

        if (isMaan) {
            // MAAN CRM - assign to MAAN tenant, NO auto-assign manager
            tenantId = await getMaanTenantId()
        } else if (formData.city) {
            // Lumara CRM - match by city/region
            ...
            if (cityTenants && cityTenants.length > 0) {
                ...
                if (!match) match = cityTenants[0]
                if (match) tenantId = match.id
            }
        }

        // Find manager (skip for MAAN — managers claim leads manually)
        if (tenantId && !isMaan) {
        ...
        const { data: lead, error } = await supabase
            ...
            .insert({
                ...
                city: formData.city || (formData.crmType === 'maan' ? 'MAAN' : 'N/A'),
                region: formData.region || null,
                services: formData.services || [],
                tenant_id: tenantId,
                assigned_manager_id: assignedManagerId,
                source: tracking.utm_source || (formData.crmType === 'maan' ? 'maan-form' : 'website'),
                status: assignedManagerId ? 'processing' : 'new',
                form_config_id: formData.formConfigId,
        ...
        await supabase.from('lead_history').insert({
            ...
            comment: `Заявка с формы (${formData.crmType.toUpperCase()})`
        })
        ...
        console.error("Submit Form Lead Error:", error)
